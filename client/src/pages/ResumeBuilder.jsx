...
import { ArrowLeftIcon, Briefcase, ChevronLeft, ChevronRight, ChevronRightIcon, Download, EyeIcon, EyeOff, FileText, GraduationCap, Share2Icon, ShareIcon, Sparkles, User, UserIcon } from "lucide-react";
...
const ResumeBuilder= () => {
    ...
    const loadExistingResume = async () => {
      try {
        ...
        if(data.resume){
            ...
            document.title = data.resume.title + " - Resume Forge"
        }
      } catch (error) {
                        ...
                        <div className="flex items-center">
                         {activeSectionIndex !== 0 && (
                            <button onClick={() => setActiveSectionIndex((prevIndex) =>
                                 Math.max(prevIndex - 1,0))} className="flex items-center gap-1 p-3 rounded-lg text-sm font-medium
  ...
  text-gray-600 hover:bg-gray-50 transition-all disabled:opacity-50"
>
  Next
  <ChevronRightIcon className="size-4" />
                       ...
                       text-green-600 ring hover:ring-green-400 transition-all rounded-md px-6 py-2 
                       mt-6 text-sm">
                        Save Changes
                       </button>
                    </div>
